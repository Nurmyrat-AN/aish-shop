class MAPPED_GET_LIST {
    ...
    mapped_data = {
        categories: async () => ({
            categories: this.sepearateList(await this.UTILS.queryAsync(`SELECT *, (SELECT json_arrayagg( brand_id ) FROM category_brand WHERE category_brand.category_id=categories.id) as brands FROM categories WHERE 
